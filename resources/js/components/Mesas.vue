<template>
    <div>
        <!-- Modal -->
        <div class="modal" id="mdlMesas" tabindex="-1" aria-labelledby="mdlMesasLabel" aria-hidden="true" @keydown.esc.prevent="cerrar()">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h6 class="modal-title" id="mdlMesasLabel">Mantenimiento de Mesas</h6>
							...
							<table class="table table-sm table-dark table-hover table-bordered" style="table-layout: fixed;">
							  	...
							  	<tbody  class="table-group-divider">
							    	<tr v-for="(i, index) in list" :ref="index" :class="n_index == index ? 'table-active' : ''" @click="selLista(index)" @dblclick="editar()">
							      		<td class="text-left" v-text="i.c_mesa"></td>
							      		<td class="text-left text-truncate" v-text="i.l_mesa"></td>
							      		<td></td>
							    	</tr>
							  	</tbody>
							</table>
                                    ...
                                    <div class="card-body">
                                        <div class="row g-0">
                                            <div class="col-2 px-1 mb-1">
                                                ...
                                                <div class="">
                                                    <input type="text" class="form-control form-control-sm" :value="f.c_mesa" disabled  />
                                                </div>
                                            </div>
                                            <div class="col-5 px-1 mb-1">
                                                <label for="" class="col-form-label col-form-label-sm">Descripción:</label>
                                                <div class="">
                                                    <input type="text" class="form-control form-control-sm text-uppercase" v-model="f.l_mesa" :disabled="disabled" />
                                                </div>
                                            </div>
                                            <div class="col-2 px-1 mb-1">
                                                <label for="" class="col-form-label col-form-label-sm">Nº de Silla:</label>
                                                <div class="">
                                                    <number type="tel" class="form-control form-control-sm text-end" v-model="f.n_sill" :precision="0" maxlength="2" :disabled="disabled" />
                                                </div>
                                            </div>
                                            <div class="col-3 px-1 mb-1">
                                                <label for="" class="col-form-label col-form-label-sm">Ubicación:</label>
                                                <div class="">
                                                    <select  class="form-control form-control-sm" v-model="f.c_ubic" :disabled="disabled">
                                                        <option value="0"></option>
                                                        <option v-for="i in ubicacion" :value="i.c_ubic" v-text="i.l_ubic"></option>
                                                    </select>
                                                </div>
                                            </div> 
                                        </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.currentModal = new coreui.Modal(document.getElementById('mdlMesas'), {
                keyboard: false,
                backdrop: 'static',
            })
        ...
        methods:{
            ...
            selLista(index) {
                ...
                this.f.c_mesa = value.c_mesa
                this.f.l_mesa = value.l_mesa
                this.f.n_sill = Number(value.n_sill)
            ...
            grabar() {
                this.q_load = 1
                axios.post("/mesas/grabmesas", {...this.f, k_grab: this.k_grab})
                .then(resul => {
