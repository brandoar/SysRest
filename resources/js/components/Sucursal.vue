<template>
    <div>
        <!-- Modal -->
        <div class="modal" id="mdlSucursal" tabindex="-1" aria-labelledby="mdlSucursalLabel" aria-hidden="true" @keydown.esc.prevent="cerrar()">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h6 class="modal-title" id="mdlSucursalLabel">Mantenimiento de Sucursales</h6>
							...
							<table class="table table-sm table-dark table-hover table-bordered" style="table-layout: fixed;">
							  	...
							  	<tbody  class="table-group-divider">
							    	<tr v-for="(i, index) in list" :ref="index" :class="n_index == index ? 'table-active' : ''" @click="selLista(index)" @dblclick="editar()">
							      		<td class="text-left" v-text="i.c_sucu"></td>
							      		<td class="text-left text-truncate" v-text="i.l_sucu"></td>
							      		<td></td>
							    	</tr>
							  	</tbody>
							</table>
                                    ...
                                    <div class="card-body">
                                        <div class="row g-0">
                                            <div class="col-2 px-1 mb-1">
                                                ...
                                                <div class="">
                                                    <input type="text" class="form-control form-control-sm" :value="f.c_sucu" disabled />
                                                </div>
                                            </div>
                                            <div class="col px-1 mb-1">
                                                <label for="" class="col-form-label col-form-label-sm">Descripción:</label>
                                                <div class="">
                                                    <input type="text" class="form-control form-control-sm text-uppercase" v-model="f.l_sucu" :disabled="disabled" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
...
<script>
    ...
    export default {
        ...
        updated() {
            ...
            this.$store.commit("mutSucursal", [])
        },
        mounted() {
            this.currentModal = new coreui.Modal(document.getElementById('mdlSucursal'), {
                keyboard: false,
                backdrop: 'static',
            })
        ...
        methods:{
            ...
            grabar() {
                this.q_load = 1
                axios.post("/sucursal/grabsucursal", {...this.f, k_grab: this.k_grab})
                .then(resul => {
            ...
            eliminar() {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        this.q_load = 1
                        axios.post("/sucursal/elimsucursal",{
                            c_sucu: value.c_sucu
                        })
                        .then(resp => {
