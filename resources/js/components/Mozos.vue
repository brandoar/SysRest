<template>
    <div>
        <!-- Modal -->
        <div class="modal" id="mdlMozos" tabindex="-1" aria-labelledby="mdlMozosLabel" aria-hidden="true" @keydown.esc.prevent="cerrar()">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h6 class="modal-title" id="mdlMozosLabel">Mantenimiento de Mozos</h6>
							...
							<table class="table table-sm table-dark table-hover table-bordered" style="table-layout: fixed;">
							  	...
							  	<tbody  class="table-group-divider">
							    	<tr v-for="(i, index) in list" :ref="index" :class="n_index == index ? 'table-active' : ''" @click="selLista(index)" @dblclick="editar()">
							      		<td class="text-left" v-text="i.c_mozo"></td>
							      		<td class="text-left text-truncate" v-text="i.l_mozo"></td>
							      		<td></td>
							    	</tr>
							  	</tbody>
							</table>
                                    ...
                                    <div class="card-body">
                                        <div class="row g-0">
                                            <div class="col-2 px-1 mb-1">
                                                ...
                                                <div class="">
                                                    <input type="text" class="form-control form-control-sm" :value="f.c_mozo" disabled />
                                                </div>
                                            </div>
                                            <div class="col px-1 mb-1">
                                                <label for="" class="col-form-label col-form-label-sm">Apellidos y Nombres:</label>
                                                <div class="">
                                                    <input type="text" class="form-control form-control-sm text-uppercase" v-model="f.l_mozo" :disabled="disabled" />
                                                </div>
                                            </div>
                                            <div class="col-2 px-1 mb-1">
                                                <label for="" class="col-form-label col-form-label-sm">Clave:</label>
                                                <div class="">
                                                    <masked-input type="text" class="form-control form-control-sm text-uppercase" v-model="f.l_clav" :disabled="disabled" 
                                                            mask="1111"
                                                            placeholder-char=" "/>
                                                </div>
                                            </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.currentModal = new coreui.Modal(document.getElementById('mdlMozos'), {
                keyboard: false,
                backdrop: 'static',
            })
        ...
        methods:{
            ...
            selLista(index) {
                ...
                this.f.c_mozo = value.c_mozo
                this.f.l_mozo = value.l_mozo
                this.f.l_clav = value.l_clav
            },
            editar() {
                this.disabled = false
                this.k_grab = 'modi'
            ...
            grabar() {
                this.q_load = 1
                axios.post("/mozos/grabmozos", {...this.f, k_grab: this.k_grab})
                .then(resul => {
